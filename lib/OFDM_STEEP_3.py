...
def get_h(n=1):
    x = np.random.normal(0, np.sqrt(0.5), size=(n)) + 1j * np.random.normal(
        0, np.sqrt(0.5), size=(n)
    )
    return x
...
def get_rate_lower(hAB, hBA, SAB, SBA, alpha, beta, nE):
    ...
    for i in range(R_lower):
        ...
        temp1 = np.log2(1 + get_SNR_main(a, b)) - np.log2(1 + get_SNR_eve(a, c, d))
        temp1 = max(0, temp1)
...
def get_ckey(hBAs, SBAs, alpha, nE):
    sssum = 0
    for i in range(len(hBAs)):
        SBA = SBAs[i]
        SEA = alpha * SBA
        bob_gain = SBA * get_gain(hBAs[i])
        ssum = 0
        for j in range(R_lower):
            hEA = get_h(nE)
            eve_gain = SEA * get_gain(hEA)
            temp1 = bob_gain / (1 + eve_gain)
            temp2 = np.log2(1 + temp1)
            ssum = ssum + temp2
        sssum = sssum + (ssum / R_lower)
    return sssum / len(hBAs)


def get_rate_upper_123(SAB_avg, SBA_avg, alpha, beta, nE, N):
...
def get_rate_upper_245(SAB_avg, SBA_avg, alpha, beta, nE, N):
    ...
    for i in tqdm(range(R_upper)):
        while True:
            try:
                ...
                SABs_new = power_alloc_Bob(
                    hABs, hBAs, SABs, SBAs, alpha, beta, nE, updt=False
                )
                policy_4[i] = get_rate_middle(
                    hABs, hBAs, SABs_new, SBAs, alpha, beta, nE
                )

                SBAs_new = power_alloc_Alice(SABs_new, SBAs)
                policy_5[i] = get_rate_middle(
                    hABs, hBAs, SABs_new, SBAs_new, alpha, beta, nE
                )

                SBAs_new = power_alloc_Alice_wf(hBAs, SABs_new, SBAs)
                policy_6[i] = get_rate_middle(
                    hABs, hBAs, SABs_new, SBAs_new, alpha, beta, nE
                )
    ...
    print((countB / (R_upper + countB)) * 100, "%")

    return np.mean(policy_2), np.mean(policy_4), np.mean(policy_5), np.mean(policy_6)


def get_rate_upper_5(SAB_avg, SBA_avg, alpha, beta, nE, N):
    ...
    for i in tqdm(range(R_upper)):
        while True:
            try:
                ...
                print(f"An error occurred: {e}")
    print((countB / (R_upper + countB)) * 100, "%")
    return np.mean(policy_5)


def get_rate_upper_50(SAB_avg, SBA_avg, alpha, beta, nE, N):
    ...
    for i in tqdm(range(R_upper)):
        while True:
            try:
                ...
                SABs_new, SBAs_new = power_alloc_classic(hABs, hBAs, SABs, SBAs)
                # print(sum(SABs_new),sum(SABs))
                # print(sum(SBAs_new),sum(SBAs))

                policy_0[i] = get_rate_middle_classic(
                    hABs, hBAs, SABs, SBAs, alpha, beta, nE
                )
...
def get_rate_upper_5689(SAB_avg, SBA_avg, alpha, beta, nE, N):

    policy_5 = np.zeros(R_upper)
    policy_6 = np.zeros(R_upper)
    policy_8 = np.zeros(R_upper)
    policy_9 = np.zeros(R_upper)
    countB = 0
    ...
    for i in tqdm(range(R_upper)):
        while True:
            try:
                ...
                SABs_new, SBAs_new = power_alloc_classic(hABs, hBAs, SABs, SBAs)
                policy_9[i] = get_ckey(hBAs, SBAs_new, alpha, nE)

                policy_8[i] = get_ckey(hBAs, SBAs, alpha, nE)

                # print(policy_8[i], policy_9[i], policy_8[i] < policy_9[i])

                SABs_new = power_alloc_Bob(
                    hABs, hBAs, SABs, SBAs_new, alpha, beta, nE, updt=False
                )
                SBAs_new = power_alloc_Alice_wf(hBAs, SABs_new, SBAs)

                # print(SBAs_new)
                # print(sum(SBAs_new),sum(SBAs),sum(SBAs_new)-sum(SBAs))

                policy_6[i] = get_rate_middle(
                    hABs, hBAs, SABs_new, SBAs_new, alpha, beta, nE
                )

                # print(policy_5[i], policy_6[i])
...
def find_SAB_for_mu(mu, hAB, hBA, SBA, p_upper):
    ...
    if function_g(hAB, hBA, p_upper, SBA) > mu:
        return p_upper
...
def power_alloc_Bob(hABs, hBAs, SABs, SBAs, alpha, beta, nE, updt=False):
    ...
    if not updt:
        lns = np.zeros(len(II))
        for i in II:
            lns[i] = get_ln(hABs[i], hBAs[i], SABs[i], SBAs[i], alpha, beta, nE)
    ...
    for count in range(max_iter):
        ...
        for i in II:
            ...
            if updt:
                ln = get_ln(hABs[i], hBAs[i], SABn, SBAs[i], alpha, beta, nE)
            else:
                ln = lns[i]
            vv = vv - ln
            if vv <= thres:
...
def power_alloc_Alice_wf(hBAs, SABs, SBAs):
    ...
    _, _, temp_SBAs = water_filling(len(temp_h), 1 / temp_h, sum(SBAs))

    SBAs_new = np.zeros(len(SBAs))
    i = 0
    for j in II:
        SBAs_new[j] = temp_SBAs[i]
        i += 1
...
def power_alloc_classic(hABs, hBAs, SABs, SBAs):
    ...
    if abs(sum(SABs) - sum(SABs_new)) > 0.001:
        raise Exception("power inequality")
    if abs(sum(SBAs) - sum(SBAs_new)) > 0.001:
        raise Exception("power inequality")

    # temp1 = SABs_new+(1/hABs)
    # if np.max(temp1)-np.min(temp1) > 1:
    #     # print(SABs_new, temp1)
    #     raise Exception("power mismatch")
    # temp2 = SBAs_new+(1/hBAs)
    # if np.max(temp2)-np.min(temp2) > 1:
    #     # print(temp2)
    #     raise Exception("power mismatch")
